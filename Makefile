...
pytest:
	PYTHONPATH=. poetry run pytest --cov-report term-missing --cov=ready tests/

flake8:
	poetry run flake8 ready/ tests/

mypy:
	poetry run mypy ready/
...
release:
	...
	make build
	poetry publish
	git add pyproject.toml ready/__version__.py
	git commit -m "Bumped version" --allow-empty
	git tag -a `python ready/__version__.py` -m `python ready/__version__.py`
	git push
	git push --tags
